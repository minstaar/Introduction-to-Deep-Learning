...
from typing import Dict, List, Callable, NewType, Optional
...
def nn_forward_backward(
    ...
    data_loss = -torch.log(probs[torch.arange(N), y]).mean()
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-5, 3e-5, 1e-4]
    learning_rate_decays = [1, 0.98, 0.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
