...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      diff = x_test_flatten - x_train_flatten[i:i+1]
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_sq = (x_train_flatten ** 2).sum(dim=1, keepdim=True)
    x_test_sq = (x_test_flatten ** 2).sum(dim=1, keepdim=True)

    dists = x_train_sq - 2 * (x_train_flatten @ x_test_flatten.T) + x_test_sq.T
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
      x_train = dists[:, j]
      _, nn_indicies = torch.topk(x_train, k, largest=False)
      nn_labels = y_train[nn_indicies]
      y_pred[j] = torch.bincount(nn_labels, minlength=num_classes).argmax()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      for i in range(num_folds):
        ...
        x_cat = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i])
        y_cat = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i])

        classifier = KnnClassifier(x_cat, y_cat)
