...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x = torch.cat(xs)
    idx = torch.repeat_interleave(torch.arange(ls.shape[0]), ls)
    sums = torch.segment_reduce(idx, x, reduce="sum")
    y = sums / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_sorted, x_sorted_indices = x.sort()
    diff = torch.diff(x_sorted, prepend=x_sorted[:1]-1)
    
    uniques = x_sorted[diff != 0]
    indices = x_sorted_indices[diff != 0]
